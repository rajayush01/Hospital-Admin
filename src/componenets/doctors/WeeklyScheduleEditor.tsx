...
import { Stack, Chip, Box, Typography, Divider, Button, CircularProgress } from "@mui/material";
    ...
    <>
      <Stack spacing={3}>
        ...
        <Box display="flex" gap={1} justifyContent="space-between" flexWrap="wrap">
          {days.map((d) => (
            <motion.div key={d.key} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
              <Chip
                label={d.label}
        ...
        >
          <Typography variant="h6" fontWeight={700} mb={1} sx={{ textTransform: "capitalize" }}>
            {activeDay}
          </Typography>
        ...
        >
          {loading ? <CircularProgress size={20} color="inherit" /> : 'Save Weekly Schedule'}
        </Button>
      </Stack>
